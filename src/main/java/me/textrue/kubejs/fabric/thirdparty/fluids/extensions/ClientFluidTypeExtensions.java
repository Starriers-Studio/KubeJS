...
public interface ClientFluidTypeExtensions {
	ClientFluidTypeExtensions DEFAULT = new ClientFluidTypeExtensions() {};

	Map<FluidType, ClientFluidTypeExtensions> FLUID_TYPE_EXTENSIONS = new Reference2ObjectOpenHashMap<>();

	static ClientFluidTypeExtensions of(FluidState state) {
		return of(state.kjs_thirdparty$getFluidType());
	}

	static ClientFluidTypeExtensions of(Fluid fluid) {
		return of(fluid.kjs$getFluidType());
	}

	static ClientFluidTypeExtensions of(FluidType type) {
		return FLUID_TYPE_EXTENSIONS.getOrDefault(type, DEFAULT);
	}
	...
	default void renderOverlay(Minecraft mc, PoseStack poseStack) {
		...
		if (texture != null)
			ScreenEffectRendererHelper.renderFluid(mc, poseStack, texture);
	}
