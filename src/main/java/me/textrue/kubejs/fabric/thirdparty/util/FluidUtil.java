...
import me.textrue.kubejs.fabric.thirdparty.fluids.capability.IFluidHandlerItem;
import me.textrue.kubejs.fabric.thirdparty.items.ItemHandler;
import me.textrue.kubejs.fabric.thirdparty.items.ItemHandlerHelper;
...
public class FluidUtil {
	...
	public static boolean interactWithFluidHandler(Player player, InteractionHand hand, IFluidHandler handler) {
		...
		EntityApiLookup<ItemHandler, Void> lookup = EntityApiLookup.get(ThirdPartyContexts.FLUID_HANDLER_ENTITY_ID, ItemHandler.class, Void.class);
		var playerInventory = lookup.find(player, null);
	 ...
	 *                    See {@link #tryFillContainerAndStow(ItemStack, IFluidHandler, ItemHandler, int, Player, boolean)}.
	 ...
	 *                         Separate handling must be done to reduce the stack size, stow containers, etc, on success.
	 *                         See {@link #tryEmptyContainerAndStow(ItemStack, IFluidHandler, ItemHandler, int, Player, boolean)}.
	 ...
	 */
	public static FluidActionResult tryFillContainerAndStow(ItemStack container, IFluidHandler fluidSource, ItemHandler inventory, int maxAmount, @Nullable Player player, boolean doFill) {
	 ...
	 * @return a {@link FluidActionResult} holding the result and the resulting container. The resulting container is empty on failure.
	 */
	public static FluidActionResult tryEmptyContainerAndStow(ItemStack container, IFluidHandler fluidDestination, ItemHandler inventory, int maxAmount, @Nullable Player player, boolean doDrain) {
		if (container.isEmpty()) {
			return FluidActionResult.FAILURE;
		}
	...
	public static Optional<IFluidHandlerItem> getFluidHandler(ItemStack itemStack) {
		ItemApiLookup<IFluidHandlerItem, Void> lookup = ItemApiLookup.get(ThirdPartyContexts.FLUID_HANDLER_ITEM_ID, IFluidHandlerItem.class, Void.class);
		IFluidHandlerItem handler = lookup.find(itemStack, null);
	...
	public static Optional<IFluidHandler> getFluidHandler(Level level, BlockPos blockPos, @Nullable Direction side) {
		BlockApiLookup<IFluidHandler, Direction> lookup = BlockApiLookup.get(ThirdPartyContexts.FLUID_HANDLER_BLOCK_ID, IFluidHandler.class, Direction.class);
		IFluidHandler handler = lookup.find(level, blockPos, side);
		return Optional.ofNullable(handler);
	}
}
