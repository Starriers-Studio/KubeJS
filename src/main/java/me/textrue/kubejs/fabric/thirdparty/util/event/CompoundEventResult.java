 ...
 * @see ThirdPartyEventResult
 */
public class CompoundEventResult<T> {
	private static final CompoundEventResult<?> PASS = new CompoundEventResult<>(ThirdPartyEventResult.pass(), null);
	private final ThirdPartyEventResult result;
	...
	public static <T> CompoundEventResult<T> interrupt(Boolean value, T object) {
		return new CompoundEventResult<>(ThirdPartyEventResult.interrupt(value), object);
	}
	...
	public static <T> CompoundEventResult<T> interruptTrue(T object) {
		return new CompoundEventResult<>(ThirdPartyEventResult.interruptTrue(), object);
	}
	...
	public static <T> CompoundEventResult<T> interruptDefault(T object) {
		return new CompoundEventResult<>(ThirdPartyEventResult.interruptDefault(), object);
	}
	...
	public static <T> CompoundEventResult<T> interruptFalse(T object) {
		return new CompoundEventResult<>(ThirdPartyEventResult.interruptFalse(), object);
	}

	private CompoundEventResult(ThirdPartyEventResult result, T object) {
		this.result = result;
		this.object = object;
	}
	...
	/**
	 * Returns the {@link ThirdPartyEventResult} view of the result, this returns the same values as
	 * {@link #interruptsFurtherEvaluation()} and {@link #value()}.
	 *
	 * @return the {@link ThirdPartyEventResult} view of the result.
	 */
	public ThirdPartyEventResult result() {
		return result;
	}
